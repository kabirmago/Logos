            ...
            {users.map(u => (
              <motion.button key={u.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <div>
                  ...
                  <p className="text-[10px] font-mono uppercase opacity-50 mt-1">{u.role} · Joined {new Date(u.timestamp).toLocaleDateString()}</p>
                </div>
            ...
            <div className="space-y-4">
              {stats?.recentActivity.map((activity, i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-[#f5f5f5] border border-black/5">
                  ...
                  <span className="text-[10px] font-mono opacity-40">{new Date(activity.timestamp).toLocaleDateString()}</span>
                </div>
              ))}
